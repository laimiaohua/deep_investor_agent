...
def duan_yongping_agent(state: AgentState, agent_id: str = "duan_yongping_agent"):
    """
    段永平风格智能体：
    - 重点看消费类、互联网等"好赛道"中，是否是少数几个真正优秀的公司
    - 是否有持续稳定的盈利能力和现金流
    - 是否估值合理，有足够的时间换空间
    """
    ...
    for ticker in tickers:
        ...
        if _looks_like_cn_or_hk_ticker(ticker):
            try:
                ...
                cn_items = get_cn_balance_sheet_line_items(ticker)
                if cn_items:
        ...
        duan_output = _generate_chinese_master_output(
            ticker=ticker,
            facts=facts,
            persona_name="Duan Yongping",
            persona_label="Duan Yongping",
            investing_style=(
                "Long-term value investing, prefers a few excellent companies in consumer and internet sectors. "
    ...
    if state["metadata"].get("show_reasoning"):
        show_agent_reasoning(analysis, "Duan Yongping Agent")
...
def _generate_chinese_master_output(
    ...
    if is_chinese:
        ...
        system_prompt = (
            ...
            f"{checklist_text}\n\n"
            "Output rules:\n"
            "- bullish: Willing to patiently hold long-term or add positions\n"
            "- bearish: Unwilling to hold or will significantly reduce positions\n"
            "- neutral: Wait and see or hold but unwilling to significantly add positions\n"
            "confidence should be an integer from 0-100.\n"
            "reasoning should be detailed and complete (200-500 characters), concise and practical, must include:\n"
            "  1. Core analysis basis: Specific data indicators, financial performance, valuation levels, etc.\n"
            "  2. Enterprise quality: Business model, brand value, moat, etc.\n"
            "  3. Long-term value: Why it's worth holding long-term, what is the core logic\n"
            "  4. Valuation judgment: Whether the current price is reasonable, what is the cost-effectiveness\n"
        ...
        default_reasoning = "Limited data available, wait and see for now."
